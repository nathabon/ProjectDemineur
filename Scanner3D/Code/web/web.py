...
def scan(frame, pscan, hauteur_cap, largeur_cap, foot):
...
def save(coord, angleObjet, Fname):
    wFileName = "fichiers_ply/" + Fname
    long = 0
...
def main(pscan, Fname, foot):
    cap = cv.VideoCapture(0)
    _, frame = cap.read()
    
    hauteur_cap, largeur_cap, canaux_cap = frame.shape
    coord, angleObjet = scan(frame, pscan, hauteur_cap, largeur_cap, foot)
    save(coord, angleObjet, Fname)
...
def index():
    ...
    <html lang="fr">
        <head>
            <meta charset="utf-8">
            <title>Scanner 3D en ligne</title>
        </head>
        <body>
            <h1>Scanner 3D en ligne</h1>
            <p>Cette opération peut prendre une à trois minutes.</p>
            <p>
                <form action="do_scan.html" method="POST">
                    <label for="fileName">Nom de votre fichier 3D</label> : <input name="fileName" type="text" required> <br/>
                    <label for="nbPixelScan">Pourcentage de scan de votre fichier 3D</label> : <input name="nbPixelScan" type="number" value=25 min=1 max="100" required> <br/>
                    <label for="foot">Combien de degrès à chaque rotation ?</label> : <input name="foot" type="number" value=3 min="1" max="360" required> <br/>
...
@bottle.route("/do_scan.html", method="POST")
def do_scan():
    fileName = bottle.request.forms.get("fileName") + ".ply"
    pixelScan = int(bottle.request.forms.get("nbPixelScan"))
    nbPixelScan = int(round(100 / int(bottle.request.forms.get("nbPixelScan"))))
    eachFoot = float(bottle.request.forms.get("foot"))
    eachFoot = (eachFoot * 500) / 360
    eachFoot = int(round(eachFoot))
    
    try:
        main(nbPixelScan, fileName, eachFoot)
        return f"""
        <!DOCTYPE html>
        <html lang="fr">
            <head>
            ...
            <body>
                ...
                <p>Nom de votre fichier : <strong>{fileName}</strong></p>
                <p>Pourcentage de pixels scannés : <strong>{pixelScan}</strong></p>
                <p><a href="fichiers_ply/{fileName}" title="fichiers_ply/{fileName}">Téléchager le fichier .ply 3D.</a></p>
            </body>
        </html>
        """
    
    except AttributeError:
        return "Veuillez connecter une caméra, ou réessayer."
